...
**Target:** PNAS  
**Status:** v8 — reviewer response revision. 2026-06-15.  
**Figures:** Fig. 0 (paired vignette), Figs. 1–5 (main), Figs. S1–S3 (SI).  
**v8 changes:** significance expanded; Codex defined at first use; §2.5 title
de-literary-ized; feature standardization disclosed (§4.3); half-life
definition + threshold sensitivity added (§4.4); causal-identification
paragraph added to Discussion; "general property" softened; selective
convergence flagged for SI relocation in response letter.
...
*Source trace:* `results/figures/fig0_paired_vignette_trace.csv`. Per-move rank and winrate data are available in the trace file. Rhythm effect sizes quoted in §1.1 were computed from per-game feature analysis (source data available upon request).
