...
**Status:** Technical note v8. 2026-06-15.  
**Figures:** Fig. 0 (paired vignette), Figs. 1-5 (main), Figs. S1-S3 (SI).  
**Scope:** Research note supporting the GameCodex interactive page and
repository results package.
...
cultural record. These findings provide a quantitative framework for
understanding how external oracles reshape cultural evolution in domains
ranging from art to scientific discovery.
...
archive (1600鈥?026), we factorize each move into separable game-phase and
...
historical envelope (mean 0.015; empirical 95% range [0.004, 0.034]). The
largest drift鈥?910s to 1920s (0.034)鈥攚as an institutional revolution
...
dynamics reveal a three-phase response鈥攕hock convergence, divergent
absorption, partial oracle re-stabilization鈥攃onsistent with the held-out
Codex operation trajectory (Codex: externalized knowledge repository; Human
Codex exceeds Frequency in 22 of 22 years; sign test *p* < 0.001). Our results establish that external oracles reshape
...
In March 2016, AlphaGo defeated Lee Sedol 4鈥? in a five-game match
...
rhythm-centroid drift across eras. These two possibilities鈥攕tructural
rewriting versus circulation acceleration鈥攁re confounded in qualitative
...
top-3鈥攖he third-ranked option by policy order (zero-indexed rank 2)鈥攁nd the
...
winrate rose from 1.4% to 99.6%. His post-78 play systematically differed
from his play in games he lost to AlphaGo (adj_opp Cohen's d = +0.19; adj_own
d = +0.32), and AlphaGo's subsequent moves became more spatially displaced
(dist_last d = +0.55). Lee appears to have entered an AI-aligned regime
...
top-1; 83% top-3). His alignment did not shift鈥攊t was high from the start. He
...
moves or games鈥攊t is that this pattern characterizes the structural response
...
0.0146 and standard deviation 0.0098. The observed historical range is [0.004, 0.034]. The AI-era transition (2010s to 2020s) has |drift| = 0.0130, with
Z-score 鈭?.16 relative to the pre-2016 distribution. The AI-era drift lies
within the historical envelope (Fig. 1).

The largest drift in our record鈥?910s to 1920s (|drift| = 0.034, 2.6脳 the
AI-era magnitude)鈥攃orresponds to the institutionalization of professional Go
...
eras (Cohen's d, E0 vs E3 = 鈭?.029). The observed AI-era change is best
...
did not change across eras (Cohen's d, E0 vs E3 = 鈭?.008). Feature-level
...
the qualifying threshold of 鈮? years with 鈮? total uses. Per-era metrics are
reported in Table 1.
...
| 1980鈥?999 | 154 | 5.7 | 0.37 | 1.1 |
| 2000鈥?015 | 87 | 2.6 | 0.67 | 1.0 |
| 2016+ | 115 | 1.5 | 0.78 | 1.1 |

Adoption lag is measured as years from first observed appearance to first year
with 鈮? unique players, among qualifying recurrent patterns. These metrics are
...
conclusion that the observed acceleration is not solely a right-censoring artifact: the probability of
reaching 鈮? unique players within 5 years rose from 0.44 (pre-1980) to 0.91
(2016+) (Fig. 2). Bootstrap resampling of the adoption lag ratio (pre-1980 /
2016+) yields 4.2脳 (95% CI [2.9脳, 5.9脳]; 5,000 resamples). Reuse density
increased 2.3脳 (95% CI [2.1脳, 2.5脳]). Pattern half-life remained
approximately 1.1 years across all eras.

Long-term adoption acceleration is robust to prefix length: prefix 10 shows
9.0鈫?.3 yr, prefix 15 shows 6.3鈫?.5 yr, prefix 20 shows 4.1鈫?.4 yr. The
additional post-2016 reduction is strongest for prefix lengths 10 and 15, but
not detectable at prefix length 20, where both 2000鈥?015 and 2016+ show
1.4-year observed lag (Supplementary Information, Fig. S2).

### 2.5 Archive composition bias and its control
...
shift, we replicated the decade drift analysis on a Japan-only subset鈥攖he only
...
The 1910s鈫?920s structural drift (0.034) was decomposed by splitting games
...
(Oteai ranking, Nihon Ki-in founding). The 2010s鈫?020s drift (0.013) occurred
...
- Pre-AI (1990鈥?015): 0.0144 [95% CI 0.0109, 0.0361]
- AlphaGo shock (2016鈥?017): 0.0066 [95% CI 0.0037, 0.0151]
- AI diffusion (2018鈥?021): 0.0167 [95% CI 0.0109, 0.0323]
- Oracle era (2022鈥?025): 0.0102 [95% CI 0.0056, 0.0212]

The Korea-coded event-stream fighting index (adj_opp 鈭?adj_own) peaked during
AI diffusion (2018鈥?021: +0.038), above its pre-AI level (2008鈥?015: +0.027).
...
*p* < 0.001), with era-level mean 螖 values showing a peak at E1, a dip at E2,
...
player (螖 < 0.001 per player; Supplementary Information, Fig. S3).
...
This distinction matters beyond Go. It raises the possibility that external
oracles in cultural evolution accelerate the discovery鈥搕ransmission鈥?persistence cycle without necessarily rewriting structural grammar. The
grammar鈥攖he coupling between game-phase position and spatial-choice鈥攊s
...
A limitation of causal interpretation should be noted. The AI era coincided
with other global changes鈥攚idespread internet access, the rise of online Go
platforms, and increased tournament prize funds鈥攚hich may have independently
accelerated pattern circulation. Our fixed-horizon and Japan-only analyses
partially address confounding from data composition, but they do not isolate
the causal effect of AI from these co-occurring trends. The acceleration we
observe should be understood as characterizing the AI era broadly, not as a
causal estimate of AI's unique contribution.

Several additional limitations apply. First, the CWI archive is Japan-dominant; while
...
CWI Professional Go Archive: 95,076 SGF records spanning 1600鈥?026, with five
...
five key features (each bounded to [0, 1] by construction; adj_opp and adj_own
are neighbor-count fractions with max 1.0, dist_last is normalized by board
diameter, is_corner_open is binary, and dens_delta is a normalized count
difference bounded to approximately [鈭?, 1]). All features share comparable
scale, and the Euclidean distance is computed on their raw values without
further standardization. The pre-2016 envelope is defined by all decade-pair
drifts where the later decade ends 鈮?2010 (N = 15 decade transitions).

### 4.4 Pattern analysis

Opening patterns are defined as the first 15-move prefix string. Qualifying
threshold: 鈮? years and 鈮? total uses across all years, yielding 546 patterns
from 74,501 unique prefixes. Adoption lag: years from first observed year to
first year with 鈮? unique players, among qualifying recurrent patterns. Reuse
density: fraction of years between first and last observed year with active
usage. Half-life: years from peak-usage year to the first subsequent year
with usage 鈮?50% of peak, observed within each pattern's tracked window.
Fixed-horizon adoption: probability of reaching 鈮? unique players within a
fixed follow-up window (1, 2, 3, or 5 years). Bootstrap CIs use 5,000
per-pattern resamples within each era. Threshold sensitivity: under
alternative thresholds (鈮? years, 鈮? uses), the acceleration pattern remains
directionally consistent but with increased sample-size variation.
...
*t*, repeated for each test year 2005鈥?026. Statistical validation: sign test
for Human Codex exceeding Frequency (22/22 years, *p* < 0.001); bootstrap CI
(5,000 resamples) on era-level 螖 values.
...
player (螖 < 0.001 per player; Fig. S3). Bootstrap CIs: reported for all key
...
with deep neural networks and tree search. *Nature* 529:484鈥?89.
doi:10.1038/nature16961

2. Silver D, Schrittwieser J, Simonyan K, et al. (2017) Mastering the game
of Go without human knowledge. *Nature* 550:354鈥?59. doi:10.1038/nature24270
...
273:457鈥?64. doi:10.1098/rspb.2005.3338
...
**Fig. 0** 鈥?Two AlphaGo Encounters as Microcosm. Paired per-move KataGo
policy rank and winrate trajectories. Left: Lee Sedol vs AlphaGo, Game 4,
2016. Right: Ke Jie vs AlphaGo Master, Game 1, 2017.  
*Source trace:* `results/figures/fig0_paired_vignette_trace.csv`. Per-move rank and winrate data are available in the trace file. Rhythm effect sizes quoted in 搂1.1 were computed from per-game feature analysis (source data available upon request).

**Fig. 1** 鈥?Century-scale structural drift envelope. (A) Decade-to-decade
...
**Fig. 2** 鈥?Fixed-horizon adoption probability by era of first appearance.
Probability of reaching 鈮? unique players within 1, 2, 3, and 5 years.  
*File:* `results/figures/fig2_fixed_horizon_adoption.png`

**Fig. 3** 鈥?Two revolutions, two mechanisms. (A) 1910s鈫?920s drift
...
**Fig. 4** 鈥?Regional event-stream three-phase distance. Five-feature
...
**Fig. 5** 鈥?Pattern adoption lag and reuse density by era of first
appearance.  
...
### S1 鈥?KataGo feature-level selective convergence
...
鈭?.008). Feature-level analysis revealed selective convergence: E3 human moves
were closer to KataGo policy than E0 human moves on adj_opp (E3 Cohen's d =
鈭?.016 vs E0 d = 鈭?.220), adj_own (E3 d = +0.007 vs E0 d = 鈭?.209), and
dens_delta (E3 d = 鈭?.043 vs E0 d = +0.216). The other two dimensions
(dist_last, is_corner_open) showed divergence or no change. KataGo model:
*kata1-b28c512nbt-s13255194368-d5935380940*, maxVisits = 100, Japanese rules,
komi 6.5.

**Fig. S1** 鈥?Feature-level human-KataGo rhythm gap by era. Positive Cohen's d
= human closer to KataGo. (File: `results/figures/figS1_katago_alignment.png`;
Data: `results/phase2/katago_alignment.csv`)

### S2 鈥?Prefix-length robustness for adoption lag

Adoption lag acceleration replicated at prefix lengths 10, 15, and 20 moves.
Prefix 10 (N = 1,877 qualifying): 9.0鈫?.3 yr. Prefix 15 (N = 546 qualifying):
6.3鈫?.5 yr. Prefix 20 (N = 177 qualifying): 4.1鈫?.4 yr. The post-2016
additional reduction is strongest for prefixes 10 and 15; at prefix 20, both
2000鈥?015 and 2016+ show 1.4-year observed lag, implying that the AI-specific
increment is undetectable at this coarser pattern granularity.

**Fig. S2** 鈥?Adoption lag by era for prefix lengths 10, 15, and 20.
(File: `results/figures/figS2_prefix_robustness.png`;
Data: `phase5_defense.py` output)

### S3 鈥?Event-stream top-player leave-one-out control

The Korea-coded event-stream fighting index (adj_opp 鈭?adj_own, 2016+) was
...
single player was 螖 < 0.001. The signal is distributed across the
...
**Fig. S3** 鈥?Korea-coded stream fighting index under per-player removal.
(File: `results/figures/figS3_player_control.png`;
Data: `phase5_defense.py` output, Section 2)

---

*v8. Repository PDF release. 2026-06-15.*
