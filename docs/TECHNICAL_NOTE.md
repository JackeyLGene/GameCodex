...
The largest drift in our record - 1910s to 1920s (|drift| = 0.034, 2.6x the
...
2016+) yields 4.2x (95% CI [2.9x, 5.9x]; 5,000 resamples). Reuse density
increased 2.3x (95% CI [2.1x, 2.5x]). Pattern half-life remained
...
*p* < 0.001), with era-level mean Delta values showing a peak at E1, a dip at E2,
...
player (Delta < 0.001 per player; Supplementary Information, Fig. S3).
...
oracles in cultural evolution accelerate the discovery-transmission-
...
with other global changes - widespread internet access, the rise of online Go
platforms, and increased tournament prize funds - which may have independently
...
difference bounded to approximately [-1, 1]). All features share comparable
...
with usage <= 50% of peak, observed within each pattern's tracked window.
Fixed-horizon adoption: probability of reaching >=3 unique players within a
...
alternative thresholds (>=2 years, >=3 uses), the acceleration pattern remains
...
(5,000 resamples) on era-level Delta values.
...
player (Delta < 0.001 per player; Fig. S3). Bootstrap CIs: reported for all key
...
Analysis scripts and derived aggregate result tables are available in this
public repository. Source SGF records are available from the CWI Professional
Go Archive (https://homepages.cwi.nl/~aeb/go/games/) subject to the archive's
terms. Parsed intermediate data and per-game feature files supporting the main
figures can be regenerated from the repository pipeline where source data are
available.
...
*Source trace:* `results/figures/fig0_paired_vignette_trace.csv`. Per-move rank and winrate data are available in the trace file. Rhythm effect sizes quoted in Section 1.1 were computed from per-game feature analysis (source data available upon request).
...
single player was Delta < 0.001. The signal is distributed across the
...
*v8. Repository technical note. 2026-06-15.*
